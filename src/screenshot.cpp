...
struct _color { u_char r, g, b; };
  ...
  {
    auto xcb_conn = xcb_connect(NULL, NULL);
    auto screen = xcb_setup_roots_iterator(xcb_get_setup(xcb_conn)).data;
  ...
  u_char* take_screenshot(std::pair<uint, uint> display_size) noexcept
  {
    ...
    auto win_pixmap = xcb_generate_id(xcb_conn);
    auto* image_reply = xcb_get_image_reply(xcb_conn, get_image_task, 0);
